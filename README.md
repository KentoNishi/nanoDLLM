...
- `dataset_mode=combined|cbt|easymath`: uses the course CBT/EasyMath datasets provided by the `cs2420_cs2823r_final_project` package (install from that directory with `pip install -e cs2420_cs2823r_final_project`). Set in `Hyperparameters` or via env var `DATASET_MODE`.
